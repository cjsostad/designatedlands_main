...
Built by grouping the intersect result by `CHA_Source_ID` (the original ECCC OBJECTID stamped into each feature before export in `create_cha.py`), then summing the overlap area and comparing it to the original CHA polygon area. The key fields are:

| Field | Description |
|-------|-------------|
| `CHA_Source_ID` | The original `OBJECTID` of the CHA polygon in the ECCC national dataset (`CriticalHabitat.gdb/CriticalHabitatArea`). Stamped into the feature class before export so it survives ArcGIS OID re-numbering and flows through all intersection outputs. Join this field to `OBJECTID` in the ECCC download to trace any row back to its source polygon. |
...
The summary tables then **re-aggregate** fragments back to the original CHA polygon level using `CHA_Source_ID` as the grouping key. `FIRST(Area_ha)` recovers the original polygon area (since all fragments carry the same value), and `SUM(Overlap_Area_ha)` totals the fragments to compute `Total_CHA_Protected_Pct`.
...
`CHA_Source_ID` is the `OBJECTID` of the polygon in ECCC's `CriticalHabitat.gdb/CriticalHabitatArea` at the time of download. `create_cha.py` stamps this value into a regular attribute field before running `FeatureClassToFeatureClass` (which would otherwise discard it by reassigning OBJECTIDs sequentially). The field then flows through `PairwiseIntersect` into all intersection outputs and summary tables.
