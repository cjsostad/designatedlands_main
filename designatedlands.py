...
class DesignatedLands:
    ...
    def dump(self, suffix="", output_gdb=None):
        ...
        for fc_name in (f"designations_planarized{suffix}", f"designations_overlapping{suffix}"):
            ...
            if not arcpy.Exists(out_fc):
                raise RuntimeError(
                    f"Export failed: expected feature class not found in output GDB: {out_fc}"
                )
    ...
    def overlay(self, in_file: str, out_file: str, in_layer: str = None, out_layer: str = None):
        ...
        ensure_file_gdb(out_file, recreate_invalid=True, logger=LOG)
        if not is_file_gdb(out_file):
            raise RuntimeError(f"Overlay output path is not a valid File GDB: {out_file}")
        arcpy.conversion.FeatureClassToFeatureClass(
